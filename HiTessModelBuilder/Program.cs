      ...
      using (var logger = new PipelineLogger(CsvFolderPath, inputFileName))
      {
        try
        {
          ...
          (RawCsvDesignData? rawCsvDesignData, FeModelContext context) =
            FeModelLoader.LoadAndBuild(StrucCsv, PipeCsv, EquipCsv, forceUboltRigid: testUboltRigid, csvDebug: true, FeModelDebug: true);
          ...
          var pipeline = new FeModelProcessPipeline(
              ...
              pipelineDebug: true, verboseDebug: true, logger: logger);
