    ...
    static void Main(string[] args)
    {
      ...
      using (var logger = new PipelineLogger(CsvFolderPath, inputFileName))
      {
        try
        {
          ...
          bool testUboltRigid = true;
          (RawCsvDesignData? rawCsvDesignData, FeModelContext context) =
            FeModelLoader.LoadAndBuild(StrucCsv, PipeCsv, EquipCsv, forceUboltRigid: testUboltRigid, csvDebug: false, FeModelDebug: false);

          // 2. 파이프라인 생성 시 Logger 인스턴스 주입 (다음 스텝에서 Pipeline 성자 수정 필요)
          ...
          {
            if (rigid.Value.ExtraData["Classification"] == "Equip")
            {
      ...
      } // <- 이 점에 로그 파일 쓰기가 완료되고 파일이 잠금 해제됨
